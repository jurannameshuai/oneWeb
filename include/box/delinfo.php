...
if (!($infoid = $db->getOne('SELECT id FROM `' . $db_mymps . 'information` WHERE id = \'' . $id . '\' AND info_level > 0'))) {
...
if (empty($manage_pwd) && !($post['ismember'])) {
	...
	if (!(empty($manage_pwd)) || ($post['ismember'] == 1)) {
		if (($post['ismember'] == 0) && (mymps_count('information', 'WHERE id = \'' . $id . '\' AND manage_pwd = \'' . md5($manage_pwd) . '\'') <= 0)) {
