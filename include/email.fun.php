...
{
	global $timestamp;
	$GLOBALS['db']->query('INSERT INTO `' . $GLOBALS['db_mymps'] . 'mail_sendlist` (email,email_subject,email_content,error,last_send,template_id)VALUES(\'' . $email_to . '\',\'' . $email_subject . '\',\'' . $email_content . '\',\'' . $error . '\',\'' . $timestamp . '\',\'' . $template_id . '\')');
}
...
function send_validate_email($uid, $userid, $email, $code)
{
	...
	if (empty($mailtmp)) {
		...
		$mail_subject = $mymps_global['SiteName'] . '用户验证';
