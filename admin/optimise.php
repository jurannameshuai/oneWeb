...
$step = array(1 => '删除过期分类信息', 2 => '删除待审分类信息', 3 => '只保留最近两个月的会员登录记录', 4 => '删除无金币消费的会员消费记录', 5 => '删除支付失败的会员支付记录', 6 => '只保留' . $mymps_mymps['cfg_record_save'] . '条短信发送记录', 7 => '只保留' . $mymps_mymps['cfg_record_save'] . '条管理员登录记录', 8 => '只保留' . $mymps_mymps['cfg_record_save'] . '条管理员操作记录', 9 => '删除会员已读短消息', 10 => '只保留一个月内的邮件发送记录', 11 => 'Mysql数据库表优化');
$here = 'Mymps系统优化';
...
if (!(empty($next))) {
	...
	case '6':
		...
		$total_count = mymps_count('sms_sendlist');

		if ($mymps_mymps['cfg_record_save'] <= $total_count) {
			$delrecord = $db->getAll('SELECT id FROM `' . $db_mymps . 'sms_sendlist` ORDER BY ID DESC LIMIT 1,' . $mymps_mymps['cfg_record_save']);
			...
			$id = substr($id, 0, -1);
			mymps_delete('sms_sendlist', 'WHERE id NOT IN (' . $id . ')');
		}
