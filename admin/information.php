...
($do_action == 'upgrade') && ($info_upgrade_time[-1] = '取消大类置顶');
($do_action == 'upgrade_list') && ($info_upgrade_time[-1] = '取消小类置顶');
($do_action == 'upgrade_index') && ($info_upgrade_time[-1] = '取消首页置顶');
...
default:
	if ($action == 'delall') {
		...
		empty($id) && write_msg('删除失败，请返回重新操作！');
		...
		mymps_delete('information', ' WHERE id IN (' . $id . ')') && write_msg('分类信息删除成功！', $url, 'mymps');
	}
	else if ($action == 'upgrade_index') {
		empty($id) && write_msg('您没有选择任何一条信息主题');
		...
		case 'post':
			...
			$upgrade_time_list = ($upgrade_type_list == 1 ? '' : ($upgrade_time_list * 3600 * 24) + $begintime);
		...
		default:
			...
			if ($mymps_global['cfg_post_editor'] == 1) {
				$acontent = get_editor('content', 'information', $post[content], '100%', '300px');
			}
			else {
				$acontent = '<textarea name="content" style="width:100%;height:300px;">' . de_textarea_post_change($post[content]) . '</textarea>';
			}
			...
			$post['part'] = 'edit';
			$post['uri'] = Rewrite('info', array('catid' => $post['catid'], 'id' => $post['id']));
			$post['submit'] = '修改';
			$here = '信息内容修改';
		...
		(!(is_array($id)) || !($do_action)) && write_msg('您没有选中任何会话！');
		...
		$ids = explode(',', $id);
		!is_array($ids) && write_msg('您没有选中任何记录！');
		...
		write_msg('信息状态转为 ' . $information_level[$action] . ' 成功！', $url, 'REcord');
	}
