		...
		<form name="Login" action="index.php?part=chk&url=<?=$url?>&go=<?=$go?>" method="post" onSubmit="return CheckForm();">
			...
			<div class="login">
				<ul>
					...
					<li>
						<img style="float:right;" src="../<?php echo $mymps_global[cfg_authcodefile]; ?>" alt="看不清，请点击刷新" class="authcode" onClick="this.src=this.src+'?'" align="absmiddle"/> <input style="float:left; width:100px; height:28px;" class="input" type="text" required name="checkcode" placeholder="验证码" title="验证码" />
					</li>
