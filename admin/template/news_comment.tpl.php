...
<table border="0" cellspacing="0" cellpadding="0" class="vbm">
    ...
    <tr align="center" bgcolor="#f5fbff">
      ...
      <td bgcolor="white"><?=$v[id]?></td>
	  <td><?=$v[userid]?></td>
	  <td bgcolor="white"><?=$v[content]?></td>
      <td><?=$v[pubtime]?></td>
      <td bgcolor="white"><?=$v[title]?></td>
      <td><?=$v[comment_level]?></td>
      <td bgcolor="white"><a href="?part=<?=$part?>&action=del&id=<?=$v[id]?>" onClick="return confirm('您确定要删除吗，如不确定请点取消')">删除</a></td>
    </tr>
<?php }?>
    </tbody>
