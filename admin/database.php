...
if ($part == 'backup') {
	...
	if ($action != 'doaction') {
		...
		if($sqlcharset==""){$sqlcharset=$dbcharset;}
		$dumpcharset = ($sqlcharset ? $sqlcharset : $dbcharset);
		//echo $dumpcharset;
		//die();
	...
	if ($action == 'dorestore') {
		...
		if (is_array($delete)) {
			...
			write_msg('恭喜，成功删除指定备份文件。', 'database.php?part=restore');
		}
		else {
...
function syntablestruct($sql, $version, $db_charset)
{
	...
	if ($sqlversion === $version) {
		return $sqlversion && $db_charset ? preg_replace(array('/ character set \\w+/i', '/ collate \\w+/i', '/DEFAULT CHARSET=\\w+/is'), array('', '', 'DEFAULT CHARSET=' . $db_charset), $sql) : $sql;
	}
		...
		while ((($currsize + strlen($tabledump) + 500) < ($sizelimit * 1000)) && ($numrows == $offset)) {
			...
			while ($row = $db->fetch_row($rows)) {
				...
				for ($i = 0; $i < $numfields; $i++) {
					//if (5.4 <= $db->version()) {
						//$t .= $comma . ($usehex && !(empty($row[$i])) && (strexists($tablefields[$i]['Type'], 'char') || strexists($tablefields[$i]['Type'], 'text')) ? '0x' . bin2hex($row[$i]) : '\'' . mysql_real_escape_string($row[$i],$db->link) . '\'');
					//}
					//else {
