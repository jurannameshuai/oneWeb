...
if (!(submit_check(CURSCRIPT . '_submit'))) {
	...
	if ($part == 'edit') {
		...
		$where .= (!(empty($areaid)) ? ' AND areaid = \'' . $areaid . '\'' : '');
		$where .= ' AND status = \'' . $status . '\'';
		$rows_num = $db->getOne('SELECT COUNT(id) FROM `' . $db_mymps . 'coupon` ' . $where);
		$param = setParam(array('part', 'title', 'userid', 'begindate', 'enddate', 'cate_id', 'areaid', 'status'));
		$begindate = (!($begindate) ? '' : date('Y-m-d', $begindate));
		$enddate = (!($enddate) ? '' : date('Y-m-d', $begindate));
